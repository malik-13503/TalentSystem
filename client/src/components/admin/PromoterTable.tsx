...
export function PromoterTable({ promoters, isLoading, isError }: PromoterTableProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/promoters/${promoterToDelete.id}`, {
        method: 'DELETE',
      });
      
      if (response.ok) {
        // Success - invalidate the promoters query to refresh the data
        queryClient.invalidateQueries({ queryKey: ['/api/promoters'] });
        
        toast({
          title: "Promoter deleted",
          description: `${promoterToDelete.firstName} ${promoterToDelete.lastName} has been removed from the system.`,
        });
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {promoterToDelete && (
                <>
                  Are you sure you want to delete {promoterToDelete.firstName} {promoterToDelete.lastName}?
                  <br /><br />
